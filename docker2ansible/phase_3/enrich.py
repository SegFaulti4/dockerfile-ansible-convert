...
from docker2ansible.phase_3.commands_config import commands_config
...
def _scenario_fit_opt_type_counts(comm_list, node_options, new_option_name, new_option):
    ...
    if not opt_value['value']:
        if node_options.get(new_option_name, None) is None:
            node_options[new_option_name] = {'type': 'BASH-WORD', 'value': '0'}
            ...
            node_options[new_option_name]['value'] = node_options[new_option_name]['value'] + ' + 0'
        else:
            raise exception.EnrichCommandException(
